 ...
 * 
 * <p>Estos tests verifican la funcionalidad principal del servicio de notificaciones,
 * incluyendo envíos exitosos y manejo de errores cuando no hay proveedor configurado.</p>
 * 
 * @author PinApp Team
 */
...
class NotificationServiceTest {
    ...
    void shouldThrowNotificationExceptionWhenProviderNotConfigured() {
        ...
        Notification smsNotification = Notification.create(recipient, "Test SMS");
    ...
    void shouldCallCorrectProviderForChannel() {
        // Arrange
        NotificationProvider smsProvider = mock(NotificationProvider.class);
        when(smsProvider.supports(ChannelType.SMS)).thenReturn(true);
        when(smsProvider.getName()).thenReturn("MockSmsProvider");
        
        NotificationResult smsResult = NotificationResult.success(
            notification.id(),
            "MockSmsProvider",
            ChannelType.SMS
        );
        ...
        // Act
        NotificationResult result = service.send(smsNotification, ChannelType.SMS);
