 ...
 * 
 * <p>
 * Esta clase se utiliza únicamente para pruebas y desarrollo.
...
public class MockNotificationProvider implements NotificationProvider {
    ...
    public NotificationResult send(Notification notification) {
        log.info("[MOCK] Simulando envío de notificación [id={}] vía {} usando '{}'",
                notification.id(), supportedChannel, providerName);

        log.debug("[MOCK] Destinatario: {}", notification.recipient());
        log.debug("[MOCK] Mensaje: {}", notification.message());
        log.debug("[MOCK] Prioridad: {}", notification.priority());

        if (shouldSucceed) {
            log.info("[MOCK] ✓ Envío simulado exitoso para notificación [id={}]", notification.id());
            ...
            String errorMsg = String.format(
                    ...
                    providerName);
            log.warn("[MOCK] ✗ Envío simulado fallido para notificación [id={}]: {}",
