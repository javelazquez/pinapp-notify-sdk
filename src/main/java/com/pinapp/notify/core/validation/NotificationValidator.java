...
/**
 * Validador de notificaciones que verifica la integridad de los datos antes del
 * envío.
 * 
 * <p>
 * Este validador implementa el principio de "Fail-Fast", deteniendo el proceso
 * en el primer error encontrado para proporcionar feedback inmediato.
 * </p>
 * 
 * <p>
...
public final class NotificationValidator {
     ...
     * 
     * <p>
     * Este método coordina todas las validaciones necesarias según el canal.
     * </p>
     * 
    ...
    public static void validate(Notification notification, ChannelType channelType) {
        ...
        log.debug("Validando notificación [id={}] para canal {}",
        ...
        log.debug("Notificación [id={}] validada exitosamente", notification.id());
    }
    ...
    private static void validateMessage(String message) {
        if (message == null || message.isBlank()) {
            ...
            log.error("Validación fallida: {}", errorMsg);
     ...
     * 
     * <p>
     * Este método usa las características de Java 21 para un switch mejorado.
     * </p>
     * 
     * @param recipient   el destinatario a validar
     * @param channelType el tipo de canal
     * @throws ValidationException si el destinatario no cumple con los requisitos
     *                             del canal
     */
    ...
    private static void validateEmailChannel(Recipient recipient) {
        ...
        if (email == null || email.isBlank()) {
            ...
            log.error("Validación EMAIL fallida: email es null o vacío");
            throw new ValidationException(errorMsg);
        }
        ...
        if (!isValidEmail(email)) {
            String errorMsg = String.format(
                    ...
                    email);
            log.error("Validación EMAIL fallida: formato inválido para '{}'", email);
            throw new ValidationException(errorMsg);
        }

        log.debug("Email '{}' validado exitosamente", email);
    }
    ...
    private static void validateSmsChannel(Recipient recipient) {
        ...
        if (phone == null || phone.isBlank()) {
            ...
            log.error("Validación SMS fallida: teléfono es null o vacío");
            throw new ValidationException(errorMsg);
        }
        ...
        if (!isValidPhone(phone)) {
            String errorMsg = String.format(
                    ...
                    phone);
            log.error("Validación SMS fallida: formato inválido para '{}'", phone);
            throw new ValidationException(errorMsg);
        }

        log.debug("Teléfono '{}' validado exitosamente", phone);
    }
    ...
    private static void validatePushChannel(Recipient recipient) {
        ...
        if (deviceToken == null || deviceToken.isBlank()) {
            ...
            log.error("Validación PUSH fallida: deviceToken no encontrado o vacío");
            throw new ValidationException(errorMsg);
        }

        log.debug("Device token validado exitosamente");
    }
    ...
    private static void validateSlackChannel(Recipient recipient) {
        ...
        if (slackChannelId == null || slackChannelId.isBlank()) {
            ...
            log.error("Validación SLACK fallida: slackChannelId no encontrado o vacío");
            throw new ValidationException(errorMsg);
        }

        log.debug("Slack channel ID validado exitosamente");
    }
     ...
     * 
     * <p>
     * Acepta números en formato internacional E.164:
     * </p>
     * <ul>
