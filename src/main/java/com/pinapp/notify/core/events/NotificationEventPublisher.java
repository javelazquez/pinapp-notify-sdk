 ...
 * 
 * <p>
 * Esta clase implementa el patrón Observer de forma thread-safe,
 * permitiendo que múltiples suscriptores reciban notificaciones sobre
 * el estado de las notificaciones en tiempo real.
 * </p>
 * 
 * <p>
 ...
 * <li><strong>Publicación Segura:</strong> Si un suscriptor lanza una
 * excepción,
 * no afecta a otros suscriptores ni al flujo principal</li>
 * <li><strong>Logging Detallado:</strong> Registra errores de suscriptores para
 * debugging</li>
 * <li><strong>Sin Dependencias Externas:</strong> Implementación pura en
 * Java</li>
 * </ul>
 * 
 * <p>
...
public class NotificationEventPublisher {
    ...
    public NotificationEventPublisher() {
        ...
        log.debug("NotificationEventPublisher inicializado");
    }
     ...
     * 
     * <p>
     * Es seguro llamar a este método desde múltiples threads concurrentemente.
     * Si el mismo suscriptor se registra múltiples veces, recibirá los eventos
     * múltiples veces (no se hace deduplicación).
     * </p>
     * 
    ...
    public void subscribe(NotificationSubscriber subscriber) {
        ...
        log.debug("Nuevo suscriptor registrado. Total de suscriptores: {}", subscribers.size());
    }
     ...
     * <p>
     * Es seguro llamar a este método desde múltiples threads concurrentemente.
     * Si el suscriptor no está registrado, este método no tiene efecto.
     * </p>
     * 
     * <p>
     * Si el mismo suscriptor fue registrado múltiples veces, solo se elimina
    ...
    public boolean unsubscribe(NotificationSubscriber subscriber) {
        ...
        if (removed) {
            log.debug("Suscriptor eliminado. Total de suscriptores: {}", subscribers.size());
        } else {
            log.debug("Intento de eliminar suscriptor no registrado");
        }
     ...
     * 
     * <p>
     * El evento se notifica síncronamente a cada suscriptor en el mismo thread.
     * Si un suscriptor lanza una excepción:
     * </p>
     * <ul>
    ...
    public void publish(NotificationEvent event) {
        ...
        if (subscribers.isEmpty()) {
            log.trace("No hay suscriptores registrados, evento no publicado: {}",
                    event.getClass().getSimpleName());
            return;
        }

        log.debug("Publicando evento {} a {} suscriptor(es)",
     ...
     * 
     * <p>
     * Este método es útil para debugging y testing.
     * </p>
     * 
     ...
     * 
     * <p>
     * Este método es útil para limpieza en tests o para resetear
     * el estado del publisher.
     * </p>
     */
    public void clear() {
        int count = subscribers.size();
        subscribers.clear();
        log.debug("Todos los suscriptores eliminados. {} suscriptor(es) fueron removidos", count);
    }
}
